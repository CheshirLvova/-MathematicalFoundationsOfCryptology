...
namespace CryptographyLabs
{
    public class DecryptMachine
    {
        ...
        public string Decrypt(string text, string key, int type = 0, int language = 0)
        {
            switch (type)
            {
                case 0:
                    text = CaesarusDec(ref text, Int32.Parse(key), language);
                    break;
            }
        ...
        public string CaesarusDec(ref string text, int key, int language)
        {
