...
{
    public class TrithemiusDecryptMachine
    {
        ...
        public string Decrypt(string text, string keyOne, string keyTwo, int type = 0, int language = 0)
        {
            switch (type)
            {
                case 0:
                    text = TrithemiusOne(ref text, Int32.Parse(keyOne), Int32.Parse(keyOne), language);
                    break;
            }
        ...
        public string TrithemiusOne(ref string text, int keyOne, int keyTwo, int language)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    newText += alphabet[(alphabet.Length + alphabet.IndexOf(text[i]) + alphabet.Length - (keyOne * i + keyTwo)) % alphabet.Length].ToString();
                }
            }
