...
namespace CryptographyLabs
{
    public class XORMachineDec
    {
        ...
        public string Decrypt(string text, string secretKey, int type = 0, int language = 0)
        {
            switch (type)
            {
                case 0:
        ...
        private string Cipher(string text, string secretKey, int language=0)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    newText += alphabet[(alphabet.Length + alphabet.IndexOf(text[i]) + alphabet.Length - (currentKey[i]% alphabet.Length)) % alphabet.Length].ToString();
                }
            }
