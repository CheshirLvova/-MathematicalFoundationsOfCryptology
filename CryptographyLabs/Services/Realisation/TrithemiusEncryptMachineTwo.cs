...
{
    public class TrithemiusEncryptMachineTwo
    {
        ...
        public string Encrypt(string text, string keyOne, string keyTwo, string keyThree, int type = 0, int language = 0)
        {
            switch (type)
            {
                case 0:
        ...
        public string TrithemiusTwo(ref string text, int keyOne, int keyTwo, int keyThree, int language)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    newText += alphabet[(alphabet.Length + alphabet.IndexOf(text[i]) + keyOne*i*i + keyTwo*i + keyThree) % alphabet.Length].ToString();
                }
            }
